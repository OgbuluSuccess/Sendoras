...
const BASE_URL =
  import.meta.env.VITE_API_URL?.replace(/\/api\/v1|\/api/g, "") ||
...
const getExamples = (apiKey) => ({
  ...
  -d '{
    ...
    "html": "<h1>Hello!</h1><p>This email was sent via ${APP_NAME} API.</p>"
  }'`,
...
func main() {
    body, _ := json.Marshal(map[string]string{
        ...
        "to":      "recipient@example.com",
        "subject": "Hello from ${APP_NAME}!",
        "html":    "<h1>Hello!</h1><p>Sent via ${APP_NAME} API.</p>",
    })
...
const DocsTab = ({ apiKeys }) => {
  ...
  const activeKey =
    apiKeys.find((k) => k.status === "active")?.key || "YOUR_API_KEY";
      ...
      <td style={{ padding: "0.6rem 0.75rem" }}>
        <span
          style={{
            ...
            borderRadius: 99,
            background: required ? "#fff1f2" : "#f0fdf4",
            color: required ? "#e11d48" : "#16a34a",
            fontWeight: 600,
          }}
        >
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="d-card">
        ...
        <div style={{ fontSize: "0.85rem", color: "#64748b", lineHeight: 1.7 }}>
          ...
          header.
          <br />
          Alternatively use HTTP Basic Auth: username ={" "}
          <code
            style={{
              background: "#f1f5f9",
              padding: "0.1rem 0.4rem",
      ...
      {/* Request fields */}
      <div className="d-card">
        <p className="d-card-title" style={{ marginBottom: "1rem" }}>
          Request Body (JSON)
        </p>
          ...
          >
            ...
            <tbody style={{ borderTop: "1px solid #e2e8f0" }}>
              {field(
                ...
                "string",
                false,
                'Sender address. e.g. "Acme <hello@yourdomain.com>". Falls back to your verified domain.',
              )}
              {field(
                "to",
                "string | array",
                true,
                "Recipient(s). A single address or array/comma-separated list.",
              )}
              ...
              {field(
                ...
                "string",
                false,
                'HTML body. Required if "text" is not provided.',
              )}
              {field(
                "text",
                "string",
                false,
                'Plain text body. Required if "html" is not provided.',
              )}
      ...
      {/* Response */}
      <div className="d-card">
        <p className="d-card-title" style={{ marginBottom: "0.75rem" }}>
          Response
        </p>
        <div
          style={{
...
const ApiKeys = () => {
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    if (!canCreateApiKeys) {
      toast.error("API key creation is available on Pro or higher.");
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.msg || "Failed to generate key.");
    }
  };
  ...
  return (
    <>
      ...
      <div className="d-page-header">
        <div>
          <h1>API Keys</h1>
          <p>Manage access tokens for the {APP_NAME} API</p>
        </div>
        <button
          className="d-btn d-btn-primary"
          onClick={() =>
            canCreateApiKeys ? setShowModal(true) : handleUpgrade()
          }
          title={
            canCreateApiKeys
        ...
        >
          <Plus size={16} />
          {canCreateApiKeys ? "Generate New Key" : "Upgrade to Generate Keys"}
        </button>
      </div>
      ...
      <div className="d-filter-tabs" style={{ marginBottom: "1.25rem" }}>
        <button
          className={`d-filter-tab${tab === "keys" ? " active" : ""}`}
          onClick={() => setTab("keys")}
        >
          <Key size={14} /> My Keys
        </button>
        <button
          className={`d-filter-tab${tab === "docs" ? " active" : ""}`}
          onClick={() => setTab("docs")}
        >
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: "0.82rem", color: "#92400e", margin: 0 }}>
                Never expose API keys in client-side code or public repos. Use
              ...
              {!canCreateApiKeys && (
                ...
                >
                  API key creation is available on Pro or Enterprise plans.
                </p>
              )}
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="d-card">
              <div className="d-empty" style={{ padding: "3rem" }}>
                ...
                <p>
                  Generate your first key to start sending emails
                  programmatically.
                </p>
                <button
                  className="d-btn d-btn-primary"
                    ...
                    <tr key={k._id}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() =>
                            setRevokeTarget({ id: k._id, name: k.name })
                          }
                          style={{ gap: "0.35rem" }}
                        >
