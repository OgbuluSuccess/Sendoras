...
import { ArrowLeft, Save, Send, Download, Upload, List, Link2, AlertCircle } from 'lucide-react';
...
const CampaignBuilder = () => {
    ...
    const [formData, setFormData] = useState({ name: '', subject: '', sender: '', content: '' });
    ...
    const handleFileUpload = async (e) => {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                if (!rows || rows.length < 2) return toast.error('File is empty', { id: tid });
                ...
                if (emailIdx === -1) return toast.error('No "email" column found', { id: tid });
    ...
    const handleDriveImport = async () => {
        ...
        try {
            ...
            toast.success(`${parsed.length} recipients loaded from Drive`, { id: tid });
        } catch (error) {
            if (error.message === 'PRIVATE_FILE') {
                toast.error('This Google Drive file is private. Open the file → Share → Anyone with the link → Viewer.', { id: tid, duration: 8000 });
            } else {
    ...
    const handleDownloadSample = async (e) => {
        ...
        try {
            ...
            xlsx.writeFile(wb, 'sendora_recipient_sample.xlsx');
            ...
            <div className="d-page-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                        ...
                        <p style={{ margin: 0, fontSize: '0.875rem', color: '#64748b' }}>Fill in the details and upload recipients</p>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                    ...
                    <button className="d-btn d-btn-primary" onClick={handleSave} disabled={loading}>
            ...
            <div className="d-grid-2-3" style={{ alignItems: 'start' }}>
                ...
                <div className="d-card">
                    ...
                    <div className="d-form">
                        ...
                        <div className="d-field" style={{ marginBottom: '0.5rem' }}>
                            <label className="d-label">Recipients <span className="req">*</span></label>
                            ...
                            {recipientSource === 'upload' ? (
                                <>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                                        <label className="d-label" style={{ margin: 0 }}>Upload File</label>
                                        <button
                                            onClick={handleDownloadSample}
                                            style={{ background: 'none', border: 'none', color: '#f97316', fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600 }}
                                        >
                                    ...
                                    <label style={{
                                        ...
                                        gap: '0.5rem', padding: '1.75rem 1rem', cursor: 'pointer',
                                        border: '2px dashed #e2e8f0', borderRadius: '12px',
                                        background: recipients.length ? '#f0fdf4' : '#f8fafc',
                                        transition: 'border-color 0.2s',
                                    }}>
                                        <input type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} style={{ display: 'none' }} />
                                        <Upload size={22} color={recipients.length ? '#10b981' : '#94a3b8'} />
                                        <span style={{ fontWeight: 600, fontSize: '0.875rem', color: recipients.length ? '#10b981' : '#64748b' }}>
                                            {fileName || 'Click to Upload List'}
                                        </span>
                                        <span style={{ fontSize: '0.78rem', color: '#94a3b8' }}>
                                            {recipients.length > 0 ? `${recipients.length} recipients loaded ✓` : 'Supports .xlsx, .xls, .csv'}
                                        </span>
                                    </label>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                                        ...
                                        <button
                                            ...
                                            style={{ flexShrink: 0, fontSize: '0.82rem', padding: '0 0.85rem' }}
                                    ...
                                    <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'flex-start', fontSize: '0.71rem', color: '#92400e', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '8px', padding: '0.4rem 0.6rem', marginTop: '0.5rem' }}>
                                        <AlertCircle size={12} style={{ flexShrink: 0, marginTop: 1 }} />
                                        <span>Drive file must be set to <strong>Anyone with the link → Viewer</strong>. Private files will be rejected.</span>
                                    </div>
                                </>
                            ) : (
                                <>
                                    ...
                                    {userLists.length === 0 ? (
                                        <div style={{ padding: '1rem', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '0.85rem', color: '#64748b' }}>
                                            You don't have any contact lists yet. Go to the <button onClick={(e) => { e.preventDefault(); navigate('/contacts'); }} style={{ background: 'none', border: 'none', color: '#f97316', cursor: 'pointer', padding: 0, fontWeight: 600 }}>Contacts page</button> to import one.
                                        </div>
                                    ) : (
                                        <select
                                            className="d-input"
                ...
                <div className="d-card" style={{ display: 'flex', flexDirection: 'column' }}>
                    <div className="d-card-header">
                        <p className="d-card-title">Email Content <span style={{ color: '#ef4444' }}>*</span></p>
                    ...
                    {formData.content && (
                        <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #e2e8f0' }}>
                            <p style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Preview</p>
                            <div
                                style={{ background: '#f8fafc', borderRadius: '8px', padding: '1rem', fontSize: '0.875rem', maxHeight: 200, overflow: 'auto' }}
