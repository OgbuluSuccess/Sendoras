      ...
      {open &&
        createPortal(
          ...
          >
            ...
            {menuItem(<Copy size={14} />, "Duplicate", () =>
              onDuplicate(campaign._id, campaign.name),
            )}

            <div
              style={{ height: 1, background: "#f1f5f9", margin: "0.15rem 0" }}
            />
            ...
            {status === "draft" &&
              menuItem(
                ...
                "Send Now",
                () => onSend(campaign._id, campaign.name),
                "#f97316",
              )}
            ...
            {isSentOrFailed &&
              menuItem(
                <RefreshCw size={14} />,
                "Resend Campaign",
                () => onResend(campaign._id, campaign.name),
                "#3b82f6",
              )}
...
const Campaigns = () => {
  ...
  const handleSend = (id, name) =>
    setConfirm({ open: true, type: "send", id, name });
  const handleDelete = (id, name) =>
    setConfirm({ open: true, type: "delete", id, name });
  const handleResend = (id, name) =>
    setConfirm({ open: true, type: "resend", id, name });
    ...
    try {
      if (type === "send") {
        ...
        setCampaigns((prev) =>
          prev.map((c) =>
            c._id === id
              ? { ...c, status: "sending", sentCount: 0, failedCount: 0 }
              : c,
          ),
        );
              ...
              >
                {logModal.loading ? (
                  ...
                  >
                    ...
                    {logModal.data.recipients?.length === 0 && (
                      <div
                        style={{
                          textAlign: "center",
                          padding: "2rem",
                          ...
                          background:
                            r.status === "failed"
                              ...
                              : r.status === "queued"
                                ...
                                : "#f8fafc",
                          borderRadius: "12px",
                          border: `1px solid ${r.status === "failed" ? "#fecaca" : r.status === "queued" ? "#fde68a" : "#e2e8f0"}`,
                          display: "flex",
                          flexDirection: "column",
                          gap: "0.5rem",
                        }}
      ...
      >
        <div className="d-filter-tabs">
          {FILTERS.map((f) => (
            <button
              key={f}
              className={`d-filter-tab${filter === f ? " active" : ""}`}
              onClick={() => setFilter(f)}
            >
              {f}
            </button>
          ...
          <table className="d-table">
            ...
            <tbody>
              {paginatedCampaigns.map((c) => {
                const badge =
                  STATUS_BADGE[c.status?.toLowerCase()] || STATUS_BADGE.draft;
                return (
                  <tr key={c._id}>
                    ...
                    <td>
                      ...
                      c.progress ? (
                        <div style={{ paddingRight: "1rem" }}>
                            ...
                            >
                              ...
                              {c.progress.failed > 0 && (
                                <span
                                  ...
                                  title={
                                    c.progress.errorLogs?.length > 0
                                      ? c.progress.errorLogs
                                          .slice(0, 3)
                                          .map(
                                            (e) => `${e.email}: ${e.message}`,
                                          )
                                          .join(" | ") +
                                        (c.progress.errorLogs.length > 3
                                          ? "..."
                                          : "")
                                      : "Failed to send"
                                  }
                                >
                                  , {c.progress.failed.toLocaleString()} failed{" "}
                          ...
                          >
                            <div
                              style={{
                                height: "100%",
                                width: `${c.progress.total ? (c.progress.sent / c.progress.total) * 100 : 0}%`,
                                background:
                                  c.status === "sent" && c.progress.failed === 0
                                    ? "#10b981"
                                    : "#3b82f6",
                            ...
                            {c.progress.failed > 0 && (
                              <div
                                style={{
                                  height: "100%",
                                  width: `${(c.progress.failed / c.progress.total) * 100}%`,
                                  background: "#ef4444",
                                  transition: "width 0.5s ease-out",
                                }}
                              />
                        ...
                        `${(c.listId.contactCount || 0).toLocaleString()} (List)`
                      ) : (
                        (c.recipients?.length?.toLocaleString() ?? 0)
                      )}
                    </td>
