    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (!rows || rows.length < 2) {
          setImporting(false);
          return toast.error("File appears to be empty or has no data rows", {
            id: tid,
          });
        }
        ...
        if (parsed.length === 0) {
          setImporting(false);
          return toast.error("No valid rows with an email address found", {
            id: tid,
          });
        }
        ...
        console.error(error);
        toast.error(error.response?.data?.msg || "Failed to import contacts", {
          id: tid,
        });
      } finally {
        setImporting(false);
  ...
  const handleDriveImport = async (e) => {
    ...
    try {
      ...
      if (error.message === "PRIVATE_FILE") {
        toast.error(
          "This Google Drive file is private. To fix: open the file in Drive → Share → Anyone with the link → Viewer.",
              ...
              <tbody>
                {listContacts.map((c) => (
                  <tr key={c._id}>
                    ...
                    <td>
                      <span
                        className={`d-badge ${c.status === "active" ? "d-badge-success" : "d-badge-danger"}`}
                      >
                        {c.status}
                      </span>
                    </td>
      ...
      <div className="d-table-wrap">
        {loading ? (
          ...
          <div className="d-empty">
            ...
            <h3>No lists yet</h3>
            <p>
              Create a list and import your contacts from a CSV or Excel file.
            </p>
            <button
              className="d-btn d-btn-primary"
              onClick={() => setShowModal(true)}
          ...
          <table className="d-table">
            ...
            <tbody>
              {visibleLists.map((l) => (
                <tr
                  key={l._id}
                  onClick={() => handleViewList(l)}
                  style={{ cursor: "pointer" }}
                  className="d-tr-hover"
                >
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            >
              Upload a CSV or Excel file, or paste a <strong>public</strong>{" "}
              Google Drive link.
            </p>
            ...
            <form onSubmit={handleDriveImport}>
              ...
              >
                ...
                />
                <span>
                  The file must be publicly shared. In Drive:{" "}
                  <strong>Share → Anyone with the link → Viewer</strong>.
                  Private files will be rejected.
                </span>
              </div>
              <button
                type="submit"
