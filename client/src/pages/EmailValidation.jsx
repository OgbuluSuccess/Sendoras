...
const EmailValidation = () => {
  ...
  const handleCheck = async (e) => {
    ...
    setResult(null);
    try {
      const { data } = await api.post("/validate/single", {
        email: email.trim(),
      });
      setResult(data);
    } catch (err) {
      toast.error(
        err?.response?.data?.msg || "Server error during validation.",
      );
    } finally {
      setChecking(false);
    }
  ...
  const handleBulkUpload = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const { data } = await api.post("/validate/bulk", {
          emails: emailList,
        });
        setBulkResults(data);
    ...
    <>
      ...
      >
        ...
        <div className="d-card">
          <p className="d-card-title" style={{ marginBottom: "0.4rem" }}>
            Single Validation
          </p>
          <p
            style={{
          ...
          >
            ...
            <ul style={{ margin: "0.5rem 0 0 1rem", lineHeight: 1.8 }}>
              ...
              <li>
                ✓ Live DNS MX record lookup (does the domain accept mail?)
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="d-card">
          ...
          >
            Upload an Excel or CSV with an <code>email</code> column (up to 500
            rows).
          </p>
              ...
              {bulkResults.map((r, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td>
                      <span className={`d-badge ${badge}`}>{r.status}</span>
                    </td>
