...
const Transactions = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const successful = (Array.isArray(data) ? data : []).filter(t => t.status === 'success');
            const usd = successful.filter(t => t.currency === 'USD').reduce((a, b) => a + (b.amount / 100), 0);
            const ngn = successful.filter(t => t.currency === 'NGN').reduce((a, b) => a + (b.amount / 100), 0);
        ...
        <>
            ...
            <div className="d-stats-grid" style={{ marginBottom: '1.5rem' }}>
                <div className="d-card d-stat-card">
                    ...
                    <div className="d-stat-icon" style={{ background: 'rgba(249,115,22,0.1)', color: '#f97316' }}>
                        <CreditCard size={20} />
                    </div>
                </div>
                <div className="d-card d-stat-card">
                    ...
                    <div className="d-stat-icon" style={{ background: 'rgba(16,185,129,0.1)', color: '#10b981' }}>
                ...
                <div className="d-card d-stat-card">
                    ...
                    <div className="d-stat-icon" style={{ background: 'rgba(59,130,246,0.1)', color: '#3b82f6' }}>
                        <DollarSign size={20} />
                    </div>
                </div>
            </div>
            ...
            {/* Transactions Table */}
            {loading ? (
                <div className="d-empty"><p>Loading transactions…</p></div>
            ) : transactions.length === 0 ? (
                <div className="d-card">
                    <div className="d-empty" style={{ padding: '3rem' }}>
                        <div className="d-empty-icon"><CreditCard size={22} /></div>
                        ...
                        <tbody>
                            {transactions.map(t => (
                                <tr key={t._id}>
                                    ...
                                    <td>{t.currency}</td>
                                    <td>
                                        <span className={`d-badge ${statusColors[t.status] || 'd-badge-neutral'}`}>
                                            {t.status}
                                        </span>
                                    </td>
