...
export async function importFromDriveUrl(shareUrl) {
    ...
    if (!fileId) throw new Error('Not a valid Google Drive link. Use a "Share" link from Google Drive.');
    ...
    try {
        ...
        throw new Error('Could not reach the Sendora server. Make sure the backend is running.');
    }

    if (response.status === 403) {
        const body = await response.json().catch(() => ({}));
        if (body.msg === 'PRIVATE_FILE') throw new Error('PRIVATE_FILE');
        throw new Error('Access denied by Google Drive. Make sure the file is publicly shared.');
    }

    if (!response.ok) {
        throw new Error(`Proxy error (${response.status}). Make sure the file is publicly shared on Google Drive.`);
    }
    ...
    if (!rows || rows.length < 2) throw new Error('The file appears to be empty or has no data rows.');
    ...
    if (emailIdx === -1) throw new Error('No "email" column found. Make sure your file has an email column header.');
    ...
    if (parsed.length === 0) throw new Error('No valid email addresses found in the file.');
    return parsed;
}
