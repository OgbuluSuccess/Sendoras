 ...
 * render.mjs
 * ─────────────────────────────────────────────────────────────────────────────
 * Standalone Node.js render script for the Sendora trailer.
 *
 * Usage:
 *   node render.mjs [desktop|mobile|both]
 *
 * Prerequisites:
 *   1. Sendora client must be running at http://localhost:5173
 *   2. Sendora server must be running at http://localhost:5000
 *   3. npm install must have been run in this directory
 *
 * How it works:
 *   1. Logs in to the Sendora API to get a JWT token
...
async function getAuthToken() {
  console.log('🔐 Logging in to Sendora API…');
  try {
    ...
    console.warn('⚠️  Login succeeded but no token returned. Recording public pages only.');
    return null;
  } catch (err) {
    console.warn('⚠️  Could not reach auth API:', err.message, '\nRecording public pages only.');
    return null;
  }
}

async function renderComposition(compositionId, outputFile, authData, serveUrl) {
  console.log(`\n🎬 Rendering: ${compositionId} → ${outputFile}`);

  // Pass the auth token as inputProps — AppFrame embeds it in the iframe URL
  // as ?__token=xxx so the Sendora client reads it before React boots.
  const inputProps = {
...
async function main() {
  ...
  if (target === 'desktop' || target === 'both') {
    await renderComposition(
      'DesktopVideo',
      path.join(OUT_DIR, 'sendoras-desktop.mp4'),
      authData,
      serveUrl,
    );
  }
  ...
  if (target === 'mobile' || target === 'both') {
    await renderComposition(
      'MobileVideo',
      path.join(OUT_DIR, 'sendoras-mobile.mp4'),
      authData,
      serveUrl,
    );
  }
