...
exports.createStripeSession = async (req, res) => {
  ...
  if (!stripe)
    return res
      .status(503)
      .json({ msg: "Stripe is not configured on the server yet." });
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: { name: `Sendhiiv ${plan.name}` },
...
exports.initializePaystack = async (req, res) => {
  ...
  if (!process.env.PAYSTACK_SECRET_KEY)
    return res
      .status(503)
      .json({ msg: "Paystack is not configured on the server yet." });
