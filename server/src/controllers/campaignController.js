      ...
      campaigns.map(async (camp) => {
        ...
        if (doc.status && doc.status.toLowerCase() === "sent") {
          ...
          doc.progress = {
            ...
            sent,
            failed,
            errorLogs: doc.deliveryLogs
              ? doc.deliveryLogs.filter((l) => l.status === "failed")
              : [],
            percentage: total
...
exports.sendCampaign = async (req, res) => {
  ...
  try {
    ...
    if (campaign.status === "Sent" && !isResend)
      return res.status(400).json({ msg: "Campaign already sent" });
    ...
    if (recipients.length === 0) {
      return res
        .status(400)
        .json({ msg: "No recipients found for this campaign" });
    }
    ...
    let senderName = campaign.senderName || appSettings.defaultSenderName;
    ...
    let fromEmail = verifiedDomain
      ? `hello@${verifiedDomain.domain}`
      : appSettings.defaultSenderEmail;
    ...
    if (campaign.recipients && campaign.recipients.length > 0) {
      ...
      recipientStatuses = campaign.recipients.map((r) => {
        ...
        return {
          ...
          error:
            logEntry && logEntry.status === "failed" ? logEntry.message : null,
      ...
      const listContacts = await Contact.find({
        list: campaign.listId._id || campaign.listId,
      })
