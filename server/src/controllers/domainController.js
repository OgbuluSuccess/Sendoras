...
const getResend = () => {
    if (!resendClient && process.env.RESEND_API_KEY && !process.env.RESEND_API_KEY.includes('your_resend')) {
        resendClient = new Resend(process.env.RESEND_API_KEY);
    }
    return resendClient;
};

const activeProviders = () => (process.env.EMAIL_PROVIDER || 'ses').toLowerCase().split(',').map(s => s.trim());
...
exports.addDomain = async (req, res) => {
    ...
    if (!domainRegex.test(domain)) {
        return res.status(400).json({ msg: 'Invalid domain format. Example: acme.com' });
    }

    try {
        const existing = await Domain.findOne({ user: req.user.id, domain: domain.toLowerCase() });
        if (existing) return res.status(400).json({ msg: 'You have already added this domain.' });
...
exports.checkVerification = async (req, res) => {
    try {
        ...
        if (domain.user.toString() !== req.user.id) return res.status(401).json({ msg: 'Not authorized' });
        ...
        if (providers.includes('ses') && domain.verificationToken) {
            try {
                ...
                const [identityResult, dkimResult] = await Promise.all([
                    ses.getIdentityVerificationAttributes({ Identities: [domain.domain] }).promise(),
                    ses.getIdentityDkimAttributes({ Identities: [domain.domain] }).promise()
                ]);
        ...
        if (providers.includes('resend')) {
            ...
            if (resend) {
                ...
                if (!domain.resendDomainId) {
                    try {
                        const { data, error } = await resend.domains.create({ name: domain.domain });
                        if (!error && data) {
...
exports.deleteDomain = async (req, res) => {
    try {
        ...
        } catch (e) { console.warn('SES delete warning:', e.message); }
        ...
        if (domain.resendDomainId) {
            ...
            } catch (e) { console.warn('Resend delete warning:', e.message); }
        }
