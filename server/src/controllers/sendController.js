...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (!to)
      return res
        .status(400)
        .json({ success: false, error: '"to" is required' });
    if (!subject)
      return res
        .status(400)
    ...
    if (!html && !text)
      return res
        .status(400)
        .json({ success: false, error: '"html" or "text" body is required' });
    ...
    if (toAddresses.length === 0)
      return res
        .status(400)
        .json({ success: false, error: 'No valid recipients in "to"' });
    ...
    if (!contentCheck.allowed) {
      return res
        .status(400)
        .json({ success: false, error: contentCheck.message });
    }
    ...
    await Promise.all(
      toAddresses.map(async (recipient) => {
        try {
          ...
          const unsubscribeUrl = `${baseUrl}/unsubscribe?email=${encodeURIComponent(recipient)}&u=${req.user._id}`;
